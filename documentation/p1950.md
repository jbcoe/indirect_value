...
It may be desirable for users to control the way memory is managed by an instance of indirect_value.
Classes such as `std::vector` and `std::string` enable this customisation by allowing an allocator 
to be specified as template argument. 
