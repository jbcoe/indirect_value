...
# Introduction
The class template, indirect_value, confers value-like semantics on a free-store allocated object. An indirect_value may hold an object of a class T, copying the indirect_value will copy the object of T, and constness will propagate from the owning classes to the indirect_value type.
...
```
However, this is boilerplate code that must be repeated per Pimpl implementation.  Worse still we are not able to benefit from the rule-of-zero [CppCoreGuidelines] to have the compiler generate the methods for use.
