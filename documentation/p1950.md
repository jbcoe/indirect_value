...
Initialising `indirect_value` to a non-empty state upon construction would incur unfortunate side
  effects:
  - This would places a requirement that the underlying type is default constructable.
  - The default constructor would have to allocate, making default construction of `indirect_value` expensive.
  - Because allocations takes place, and can fail, the constructor could not now be marked as `noexcept`.

These costs would mean that `indirect_value` would always be in a non-empty state.  This removes the requirement on
pre-conditions on observable methods to avoid undefined behaviour.  The costs of a lack of null state could be
avoided by way of `optional<indirect_value<T>>`. This could be taken further to offer a specialisation of `optional`
for `indirect_value` that used the internal pointer of `indirect_value` to determine the optional status, thus
providing the null state at no additional memory cost.  However specialisation with in the Standard library have a
 poor history.


 
 
The design of `indirect_value` is impacted by how null state is represented.
