...
## Should default constructed state be a null state?
...
## Vocabulary - 'pointers' and 'values'

## Allocators 
Discuss why we do not support allocators. Copier and deleter can give us all we need that allocators can do.

For custom allocators we should guarantee that the allocator is used, for std::allocator we can use an SBO.
