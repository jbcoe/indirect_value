...
* Effects: If `p` is empty, assigns an empty object. Otherwise creates an object that owns a copy of the object managed by `p`. If `p` has a custom copier and deleter then the copy is created by the copier in `p`, and the copier and deleter of `*this` are copied from those in `p`. Otherwise, the resource managed by `*this` is initialised by the copy constructor of the resource managed by `p`.
