...
### Issues with `unique_ptr<impl>` implementation of PImpl
#### const-propagation
#### copies

### Implementation with `std::shared_ptr<impl>`
Shared mutable state is not acceptable.
TODO

### Implementation with `std::shared_ptr<const impl>`
Could be good - copies are cheap but mutating methods that modify the impl class instance cannot be implemented.
TODO

### Implementation with `polymorphic_value<impl>`
Incurs type-erasure cost for copies which we do not need to pay. Support for polymorphism is not free - we 
do not want to pay for what we don't use.
TODO

### Implementation with `indirect_value<impl>`
Essentially polymorphic_value without the polymorphism - perfect for our PImpl.
...
# Appendix A - On type erasure in C++
TODO