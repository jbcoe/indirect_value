...
Using std::unique_ptr the exposed class gets compiler generated move semantics, but not copy semantic.  If these are required the user must now manually define a copy-constructor and assignment operator.
...
{
    if (rhs.pimpl) { 
      pimpl = std::unique_ptr<impl>(*pimpl);
    }
}
...
```
However, this is boiler plate code that must be repeated per Pimpl implementation.  Worse still we are not able to benefit from the rule-of-zero to have the compiler generate this method for use.

Using the proposed indirect_value Pimpl classes could be implemented to take advantage of the rule-of-zero to provide compiler generated method implementations.  Note this is implementations only, function declarations are required in the header and in the source file with the <code>default</code> keyword in-order to instruct the compiler to generate the function bodies in the source translation unit where the definition of the implementation class is visible to the compiler. 
...
Now the external class has compiler generated copy and move constructors, copy and move assignment operators and destructor.  All of these compiler-generated functions will behave correctly.

## Deep Copying

## Why polymorphic_value does not solve this problem, i.e. not paying for what you don't use.
